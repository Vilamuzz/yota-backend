...
type Service interface {
	...
	ExportAdminFosterChildrenExpenseCSV(ctx context.Context, fosterChildrenID string, params FosterChildrenExpenseExportParams) ([]byte, string, error)
}
...
func (s *service) ExportAdminFosterChildrenExpenseCSV(ctx context.Context, fosterChildrenID string, params FosterChildrenExpenseExportParams) ([]byte, string, error) {
	ctx, cancel := context.WithTimeout(ctx, s.timeout)
	defer cancel()
	...
	expenses, err := s.repo.FindAllAdminFosterChildrenExpensesForExport(ctx, fosterChildrenID, params)
	if err != nil {
		logrus.WithFields(logrus.Fields{
			"component":          "foster_children_expense.service",
